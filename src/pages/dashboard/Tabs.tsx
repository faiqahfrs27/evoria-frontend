  ...
  return (
    <div>
      ...
      <DashTable
        ...
        emptyMessage={
          !selectedEventId
            ? "Select an event to view pending payments."
            : "No pending payments."
        }
        minWidth={800}
      />
...
export function CreateEventTab() {
  ...
  const updateForm = (key: keyof typeof form, value: string) => {
    setForm((prev) => {
      ...
      if (key === "totalSeats") {
        ...
        if (prev.isFree === "true")
          setTicketTypes([
            { name: "General Admission", price: "0", quota: value },
          ]);
      }
      return next;
    });
  ...
  const addTicketType = () =>
    setTicketTypes((prev) => [
      ...prev,
      { name: "", price: form.isFree === "true" ? "0" : "", quota: "" },
    ]);
  const removeTicketType = (index: number) =>
    setTicketTypes((prev) => prev.filter((_, i) => i !== index));
  const totalTicketQuota = ticketTypes.reduce(
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: "grid", gap: 18 }}>
          ...
          <div style={cardStyle}>
            ...
            >
              <div>
                <p style={{ color: "#F9F3E8", fontSize: 14, marginBottom: 4 }}>
                  Ticket Types
                </p>
          ...
          >
            <button
              ...
              style={{ ...ghostBtn, padding: "11px 18px" }}
            >
              Cancel
            </button>
            <button
...
export function CreateVoucherTab({ events }: { events: any[] }) {
  ...
  const createVoucherMutation = useMutation({
    mutationFn: async () =>
      axiosInstance.post(`/events/${form.eventId}/vouchers`, {
        code: form.code,
    ...
    <div style={{ maxWidth: 860 }}>
      <div style={cardStyle}>
        <div style={{ marginBottom: 22 }}>
          ...
          <p style={{ color: "#6F6F7D", fontSize: 13 }}>
            Create a discount voucher for one of your events.
          </p>
        </div>
        {events.length === 0 ? (
          <div
          ...
          >
            ...
            <button
              ...
              style={{ ...goldBtn, width: "auto", padding: "10px 16px" }}
            >
              Create Event
            </button>
          </div>
...
export function VouchersTab({ events }: { events: any[] }) {
  ...
  const { data: vouchers = [], isLoading } = useQuery({
    ...
    queryFn: async () =>
      (await axiosInstance.get(`/events/${selectedEventId}/vouchers`)).data
        .data ?? [],
